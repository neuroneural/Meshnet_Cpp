...
af::array convolve3d(af::array &signal, af::array &row_indices, af::array &col_indices, 
    ...
    for(int i = 0; i < out_channels; i++){
        ...
        for(int j = 0; j < depth; j++){
            ...
            if(j < dilation){
                output(slice) += matmul(sparse_matrix_first, 
                                        af::join(0, signal(slice, i), 
                                                    signal(operator+(slice, dilation * out_slice_dims), i)));
            }
                ...
                output(slice) += matmul(sparse_matrix_middle, 
                                        af::join(0, signal(operator-(slice, dilation * out_slice_dims), i), signal(slice, i), 
                                                    signal(operator+(slice, dilation * out_slice_dims), i)));
            }
            else{
                if(j == depth - dilation){
                ...
                output(slice) +=
                            matmul(sparse_matrix_last, af::join(0, signal(operator-(slice, dilation * out_slice_dims), i), signal(slice, i)));
...
af::array meshnet(af::array &signal, int n_layers){
    ...
    for(int i = 0; i < n_layers; i++){
       ...
       af_print(af::transpose(row_indices))
       af_print(af::transpose(col_indices))
       return output;
       af_print(col_indices)
