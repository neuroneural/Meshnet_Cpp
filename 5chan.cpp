...
af::array convolve3d(const af::array &signal, const af::array &filter,
    ...
    // af::dim4 odims( outputWidth * outputHeight * outputDepth, fDims[0] * fDims[1] * fDims[2], 1, (signal.dims())[3]);
...
af::array ELU(af::array &signal){
    signal = af::operator+(af::exp(af::operator*(signal, af::operator<=(signal, 0.0)))-1, af::operator*(signal, af::operator>(signal, 0.0)));
    signal.eval();
    return signal;
}

af::array lastlayer_argmax(af::array &in, const af::array &signal, const af::array &filter, const af::array &bias,
                          const dim4 &strides, const dim4 &padding,
    ...
    for(int i = 0; i<nfilt; i++){
        in = signal(span, span, span, 0);
        cout<<signal.dims()<<"\n\n";
        convolved = convolve3d(in, filter(span, i, 0), strides, padding, dilation, filter_dims, outArray, inPtr, outPtr);     
        for(int j = 1; j<nchl; j++){
            in = signal(span, span, span, j);
            convolved = af::operator+(convolved, convolve3d(in, filter(span, i, j), strides, padding, dilation, filter_dims, outArray, inPtr, outPtr));
            convolved.eval();
        }        
...
af::array meshnet(af::array &signal, int layers){
    ...
    af::array outArray = af::array(odims);
    af::array in = af::array(sdims);
    int dp[] = {1, 2, 4, 8, 16, 8, 4, 2, 1};
    int undim, val, chdim, nfdim;
    string path;

    float *outPtr = outArray.device<float>();
    float *inPtr = in.device<float>();
    cout<<outPtr<<inPtr;
    ...
    for(int i = 0; i<layers; i++)
    {
       ...
       if(i == 0){   
        in = signal(span, span, span, 0).copy();
        convolved = convolve3d(in, filter, strides, padding, dilation, filter_dims, outArray, inPtr, outPtr);
        ...
        for(int j = 0; j<chdim; j++){
            if(j==0){
                in = signal(span, span, span, j).copy();
                convolved = convolve3d(in, filter(span, span, j), strides, padding, dilation, filter_dims, outArray, inPtr, outPtr);
                // cout<<"Channel_0"<<"\n";
                continue;
            }
            in = signal(span, span, span, j).copy();
            convolved = af::operator+(convolved, convolve3d(in, filter(span, span, j), strides, padding, dilation, filter_dims, outArray, inPtr, outPtr));
            convolved.eval();
            // cout<<"Channel_"<<j<<"\n";
        }
        convolved = af::operator+(convolved, bias);
        ...
        o = lastlayer_argmax(in, signal, filter, bias, strides, padding, dilation, filter_dims, outArray1, inPtr, outPtr);
       }   
       signal = convolved;
       cout<<"Layer_"<<i<<"\n";
    }   
